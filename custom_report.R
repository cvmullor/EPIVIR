...
if (length(args) < 2) {
  ...
  platform <- args[2]  # illumina / nanopore
  RUN      <- args[3]  # e.g.: "250203_EPIM197_RSV"
  basedir0 <- args[4]  # directorio con los resultados de EPIVIR
  epidir   <- args[5]  # directorio EPIVIR
  workdir  <- args[6]  # directorio "work"
}
...
if ( virus == "RSV") {
  if (platform == "illumina") {
    ...
    # Seleccionar columnas
    df1 <- f1[, c('Sample', 'IRMA_consensus_N_count', 'Raw_num_reads', 'Filt_num_reads',
                  'IRMA_subtype', 'clade', 'Nextclade_qc.overallScore', 'Nextclade_qc.overallStatus')]
    
    # Convertir campos num. lecturas a numérico
    df1$Raw_num_reads <- as.integer(gsub(",", "", df1$Raw_num_reads))
    df1$Filt_num_reads <- as.integer(gsub(",", "", df1$Filt_num_reads))
    ...
    ### 4) Extraer resultados Kraken2 que no se incluyen en Summary Report (nanopore) ###
    
    indir <- "kraken2/"
    afile <- "kraken2_report.tsv"
    
    fk1 <- read.table(paste0(basedir, indir, afile), sep="\t", header = T, row.names = 1) # read table
    
    dfk1 <- cbind(rownames(fk1), fk1[,c(2,1)])
    colnames(dfk1) <- c("Sample", "percent_Respiratory.Syncityal.Virus", "percent_Homo.sapiens")
    
    
    ### 5) Merge results (nanopore) ###
    dim(df1)
    dim(df2)
    dim(df3)
    dim(dfk1)
    ...
    # Merge tables by sample name
    df4.0 <- merge(df1, df2, by.x="Sample", by.y="Sample", all = TRUE)
    df4.1 <- merge(df4.0, df3, by.x="Sample", by.y="Sample", all = TRUE)
    df4   <- merge(df4.1, dfk1, by.x="Sample", by.y="Sample", all = TRUE)
    
    df4[df4==""] <- NA # replace blank space with NA
    
    
    ### 6) Generar columnas vacías / con valores constantes ###
    nr <- nrow(df4)
    ...
    percent_missed_reads <- 100 - round(( as.numeric(df4$Mapped_Reads) / df4$Filt_num_reads)*100, 2)
    ...
    df5 <- as.data.frame(cbind(
      ID, df4$Sample, SIP, Hospital, Reception.date, Hospital.date,
      Relevance, Comments, Gender, Age, Status, df4$Coverage,
      df4$Mean_Depth, runid, df4$IRMA_consensus_N_count, df4$Raw_num_reads,
      df4$Filt_num_reads, df4$Mapped_Reads, round(percent_missed_reads, 2),
      df4$percent_Respiratory.Syncityal.Virus, df4$percent_Homo.sapiens, 
      df4$IRMA_subtype, df4$clade, version, 
      df4$Nextclade_qc.overallScore, df4$Nextclade_qc.overallStatus,
      df4$totalFrameShifts, df4$frameShifts, df4$aaInsertions, GISAID))
...
############# FLU #################
  ...
  if (platform == "illumina") {
    ...
    dfk1 <- cbind(rownames(fk1), fk1[,c(2,3,1)])
    colnames(dfk1) <- c("Sample", "percent_Influenza.A", "percent_Influenza.B", "percent_Homo.sapiens")
    ...
    # Merge tables by Sample name
    df4.0 <- merge(df1, df2.def, by.x="Sample", by.y="Sample", all = TRUE)
    df4.1 <- merge(df4.0, df3, by.x="Sample", by.y="Sample", all = TRUE)  # Merge with nextclade results (df3)
    df4   <- merge(df4.1, dfk1, by.x="Sample", by.y="Sample", all = TRUE) # Merge with kraken2 results (dfk1)
    ...
    percent_missed_reads <- 100 - round(( as.numeric(df4$mapped_reads) / df4$Filt_num_reads)*100, 2)  # Calcular % missed reads
    ...
    df5 <- cbind(ID, df4$Sample, SIP, Hospital, Reception.date, Hospital.date,
                 Relevance, Comments, Gender, Age, Status_Genomic, Status_HA.NA, df4[,13:20],
                 df4[21:28], runid, df4$IRMA_consensus_N_count, df4$Raw_num_reads,
                 df4$Filt_num_reads, df4$mapped_reads, percent_missed_reads, df4[37:39],
                 df4[,5:8], df4$clade.y, df4$subclade.y, version, df4$qc.overallScore, 
                 df4$qc.overallStatus, df4[,34:36], GISAID)
    ...
    segment_names <- c("HA", "MP", "NA", "NP", "NS", "PA", "PB1", "PB2")
    df2 <- as.data.frame(rownames(f2))
    df2[,2:9] <- NA # create 8 empty columns
    colnames(df2) <- c("Sample", segment_names)
    rownames(df2) <- df2$Sample
    ...
    ## Save new report
    df6 <- cbind(ID, df5.0$Hospital.sample.number, SIP, Hospital, Reception.date, Hospital.date,
                 Relevance, Comments, Gender, Age, df5.0$Status_Genomic, df5$Status_HA.NA, df5.0$Genomic_Cov, df4[,13:20],
                 df4[21:28], runid, df4$IRMA_consensus_N_count, df4$Raw_num_reads,
                 df4$Filt_num_reads, df4$mapped_reads, percent_missed_reads, df4[37:39],
                 df4[,5:8], df4$clade.y, df4$subclade.y, version, df4$qc.overallScore, 
                 df4$qc.overallStatus, df4[,34:36], GISAID)
